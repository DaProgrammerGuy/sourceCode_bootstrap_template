...
class courseController extends Controller
{
    ...
    public function edit(Course $course)
    {
        //
        $mainCategories = Category::whereNull('parent_id')->get();
        $subCategories = Category::where('parent_id', $course->main_category_id)->get();
        
        return view('courses.edit', compact('course', 'mainCategories', 'subCategories'));
    }
    ...
    public function update(Request $request, Course $course)
    {
        $validated = $request->validate([
            ...
            'course_methodology' => 'required|integer', // 1=Classroom, 2=Skill Dev, etc.
            'course_type' => 'required|integer',        // 1=Live, 2=On Demand, etc.
            'course_level' => 'required|integer|min:1|max:5',
            'course_subscription_method' => 'nullable|integer|min:1|max:5',
            'course_duration' => 'required|numeric',
            'course_fee' => 'nullable|numeric',
            'discount_price' => 'nullable|numeric',
        ...
        if ($request->hasFile('course_thumbnail')) {
            ...
            if ($course->course_thumbnail && file_exists(public_path('storage/' . $course->course_thumbnail))) {
                @unlink(public_path('storage/' . $course->course_thumbnail));
            }
        ...
        if ($request->hasFile('course_desktop_cover_image')) {
            if ($course->course_desktop_cover_image && file_exists(public_path('storage/' . $course->course_desktop_cover_image))) {
                @unlink(public_path('storage/' . $course->course_desktop_cover_image));
            }
        ...
        if ($request->hasFile('course_mobile_cover_image')) {
            if ($course->course_mobile_cover_image && file_exists(public_path('storage/' . $course->course_mobile_cover_image))) {
                @unlink(public_path('storage/' . $course->course_mobile_cover_image));
            }
     ...
     public function destroy(Course $course)
    {
        try {
            ...
            if ($course->course_desktop_cover_image) {
                Storage::disk('public')->delete($course->course_desktop_cover_image);
            }
            if ($course->course_mobile_cover_image) {
                Storage::disk('public')->delete($course->course_mobile_cover_image);
            }
